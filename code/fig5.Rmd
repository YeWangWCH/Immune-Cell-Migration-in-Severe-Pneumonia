...
ggsave("fig5/5g_DimPlot_plasma_disease_isotype.pdf",p, width = 7, height = 4)
```

# fig5h piled bar cell num
```{r}
plasma <- scRNAsubB[,scRNAsubB$cell_type_B %in% c("PC.1","PC.2","PC.3","PB")]
plasma <- plasma[,plasma$disease == "Pneumonia"]
...
p =ggplot(clusters) + 
  ...
  scale_y_continuous(expand = c(0,0))+
  coord_flip()
ggsave("fig5/5h_plasma_disease_celltype_isotypye_num.pdf",p,width = 5, height = 4)
```
...
ggsave("fig5/5i_Vln_plasma_disease.pdf",p,width = 4, height = 4.3)
```
...
ggsave("fig5/5j_plasma_disease_migration.pdf",p,width = 5, height = 4)
```
