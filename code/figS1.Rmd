...
# figS1a VlnPlot
```{r}
p1= VlnPlot(scRNA,"nFeature_RNA",group.by = "orig.ident",pt.size = 0,cols = color_origID)+
  NoLegend()+labs(y = "nFeature_RNA")+ 
  theme(plot.title = element_blank(),axis.title.x = element_blank(),
        axis.text.x = element_blank(), axis.ticks.x = element_blank(),
        axis.text = element_text(size = 14),axis.title = element_text(size = 14))

p2= VlnPlot(scRNA,"percent.rb",group.by = "orig.ident",pt.size = 0,cols = color_origID)+
  NoLegend()+labs(y = "percent.rb")+   
  theme(plot.title = element_blank(),axis.title.x = element_blank(),
        axis.text.x = element_blank(), axis.ticks.x = element_blank(),
        axis.text = element_text(size = 14),axis.title = element_text(size = 14))

p3= VlnPlot(scRNA,"percent.mt",group.by = "orig.ident",pt.size = 0,cols = color_origID)+
  NoLegend()+labs(y = "percent.mt")+
  theme(plot.title = element_blank(),axis.title.x = element_blank(),
        axis.text = element_text(size = 14),axis.title = element_text(size = 14))

p = p1+p2+p3
ggsave("figS1/S1a_vln_gene&mt&rb.pdf",p,width = 16,height = 6)
```
...
write.csv(all.markers, "sTable/s1b_all_markers_all_MAST.csv", row.names = F)
write.csv(top10, "sTable/s1b_top10_markers_all_MAST.csv", row.names = F)
```
...
ggsave("figS1/S1b_featureplot.png", plot = p, width = 35 , height = 15) 

#plot the legend
p = FeaturePlot(scRNA,cols = beach, features = "CD3E", raster = F)
l=as_ggplot(get_legend(p)) 
ggsave('figS1/S1b_featureplot_Legend.pdf', l)
```

# figS1c piled cell num bar
```{r}
...
ggsave("figS1/S1c_celltype_orig_num.pdf",p,width = 6, height = 5)
```
...
l=as_ggplot(get_legend(p)) 
ggsave('figS1/S1c_celltype_orig_num_legend.pdf', l, width = 4,height = 3)
```
...
ggsave("figS1/S1d_box_prop_celltype_main_PBMC.pdf", plot = p, width = 6 , height = 5)
```
